...
def generate_distinct_shades_from_cmap(
    cmap_name, num_shades, min_intensity=0.2, max_intensity=1.0
):
    """
    ...
    return [
        mcolors.to_hex(
            cmap(
                min_intensity + (i / (num_shades - 1)) * (max_intensity - min_intensity)
            )
        )
...
def plot_tank_size_correction_factors(top_dir):
    ...
    for filename in os.listdir(f"{top_dir}/tables"):
        ...
        if result:
            ...
            tank_size_factors_dfs.append(
                tank_size_factors_df.assign(vessel_range=vessel_range)
            )
            vessel_ranges.append(vessel_range)
    ...
    for fuel in fuels:
        ...
        for vessel_class in vessel_class_titles:
            vessel_data = fuel_data[
                fuel_data["Vessel Class"] == vessel_class
            ].sort_values(by="vessel_range")
            ax.plot(
                vessel_data["vessel_range"],
                vessel_data["Total"],
                label=vessel_class_titles[vessel_class],
...
def plot_cargo_loss(top_dir):
    ...
    for filename in os.listdir(f"{top_dir}/tables"):
        ...
        if result:
            ...
            modified_capacity_dfs.append(
                modified_capacity_df.assign(vessel_range=vessel_range)
            )
            vessel_ranges.append(vessel_range)
    ...
    for fuel in fuels:
        ...
        for vessel_class in vessel_class_titles:
            vessel_data = fuel_data[fuel_data["Vessel"] == vessel_class].sort_values(
                by="vessel_range"
            )
            ax1.plot(
                vessel_data["vessel_range"],
