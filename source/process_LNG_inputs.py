...
def main():
    ...
    df_pivot = df_grouped.pivot_table(
        index="Stage", columns="Input", values="Value", aggfunc="sum"
    ).reset_index()
    ...
    # Apply conversions
    df_pivot["NG Consumption (kg/kg)"] = df_pivot["NG Consumption (Btu/mmBtu)"].apply(
        Btu_per_mmBtu_to_kg_per_kg
    )
    df_pivot["NG Consumption (GJ/kg)"] = df_pivot["NG Consumption (kg/kg)"].apply(
        kg_per_kg_to_GJ_per_kg
    )
    df_pivot["Water Consumption (m^3/kg)"] = df_pivot[
        "Water Consumption (gal/mmBtu)"
    ].apply(gal_per_mmBtu_to_m3_per_kg)
    df_pivot["Electricity Consumption (kWh/kg)"] = df_pivot[
        "Electricity Consumption (Btu/mmBtu)"
    ].apply(Btu_per_mmBtu_to_kWh_per_kg)
    df_pivot["CH4 Emissions (kg/kg)"] = df_pivot["CH4 Emissions (g/mmBtu)"].apply(
        g_per_mmBtu_to_kg_per_kg
    )
    df_pivot["CO2 Emissions (kg/kg)"] = df_pivot["CO2 Emissions (g/mmBtu)"].apply(
        g_per_mmBtu_to_kg_per_kg
    )
    ...
    df_final = df_pivot[
        [
            "Stage",
            "NG Consumption (kg/kg)",
            "NG Consumption (GJ/kg)",
