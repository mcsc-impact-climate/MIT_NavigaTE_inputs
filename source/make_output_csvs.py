...
quantities = [
    'ConsumedEnergy_lsfo', 'ConsumedEnergy_main', 'TotalCAPEX', 'TotalExcludingFuelOPEX', 'TotalFuelOPEX',
    'TotalCost', 'TotalEquivalentWTT', 'TotalEquivalentTTW', 'TotalEquivalentWTW'
]
...
def read_results(fuel, fuel_type, pathway, country, number, filename, all_results_df):
    ...
    for vessel_type in vessels:
        for vessel in vessels[vessel_type]:
            ...
            results_dict['TotalEquivalentWTT'] = float(results_df_vessel['TotalEquivalentWTT'].loc['2024-01-01'])
            results_dict['TotalEquivalentTTW'] = float(results_df_vessel['TotalEquivalentTTW'].loc['2024-01-01'])
            results_dict['TotalEquivalentWTW'] = float(results_df_vessel['TotalEquivalentWTW'].loc['2024-01-01'])
            results_dict['TotalCAPEX'] = float(results_df_vessel['TotalCAPEX'].loc['2024-01-01'])
            results_dict['TotalFuelOPEX'] = float(results_df_vessel['TotalFuelOPEX'].loc['2024-01-01'])
            results_dict['TotalExcludingFuelOPEX'] = float(results_df_vessel['TotalExcludingFuelOPEX'].loc['2024-01-01'])
            results_dict['TotalCost'] = float(results_df_vessel['TotalCAPEX'].loc['2024-01-01']) + float(results_df_vessel['TotalFuelOPEX'].loc['2024-01-01']) + float(results_df_vessel['TotalExcludingFuelOPEX'].loc['2024-01-01'])
            results_dict['Miles'] = float(results_df_vessel['Miles'].loc['2024-01-01'])
            if vessel_type == 'container':
                results_dict['CargoMiles'] = float(results_df_vessel['CargoMiles'].loc['2024-01-01']) * TONNES_PER_TEU
            elif vessel_type == 'gas_carrier':
                results_dict['CargoMiles'] = float(results_df_vessel['CargoMiles'].loc['2024-01-01']) * TONNES_PER_M3_LNG
            else:
                results_dict['CargoMiles'] = float(results_df_vessel['CargoMiles'].loc['2024-01-01'])
    
            if fuel == 'lsfo':
                results_dict['ConsumedEnergy_main'] = float(results_df_vessel['ConsumedEnergy_lsfo'].loc['2024-01-01'])
                results_dict['ConsumedEnergy_lsfo'] = float(results_df_vessel['ConsumedEnergy_lsfo'].loc['2024-01-01']) * 0
            else:
                results_dict['ConsumedEnergy_main'] = float(results_df_vessel[f'ConsumedEnergy_{fuel}'].loc['2024-01-01'])
                results_dict['ConsumedEnergy_lsfo'] = float(results_df_vessel['ConsumedEnergy_lsfo'].loc['2024-01-01'])
...
def collect_all_results(top_dir):
    ...
    columns = ['Vessel', 'Fuel', 'FuelType', 'Pathway', 'Country', 'Number', 'TotalEquivalentWTT', 'TotalEquivalentTTW', 'TotalEquivalentWTW', 'TotalCAPEX', 'TotalFuelOPEX', 'TotalExcludingFuelOPEX', 'TotalCost', 'Miles', 'CargoMiles', 'ConsumedEnergy_main', 'ConsumedEnergy_lsfo']
    ...
    all_results_df = read_results('lsfo','grey', 'fossil', 'Global', 1, results_filename, all_results_df)
    ...
    for fuel_pathway_country in fuel_pathway_country_tuples:
        ...
        all_results_df = read_results(fuel, fuel_type, pathway, country, number, results_filename, all_results_df)
...
def add_number_of_vessels(all_results_df):
    ...
    all_results_df['n_vessels'] = all_results_df['base_vessel_name'].map(vessel_size_number).astype(float)
...
def add_evaluated_quantities(all_results_df):
    for quantity in quantities:
        for evaluation_choice in evaluation_choices:
            ...
            all_results_df[f'{quantity}_{evaluation_choice}'] = all_results_df[quantity] / all_results_df[column_divide] if column_divide else all_results_df[quantity]
...
def add_fleet_quantities(all_results_df):
    for quantity in quantities + ['Miles', 'CargoMiles']:
        ...
        all_results_df[f'{quantity}_fleet'] = all_results_df[quantity] * all_results_df['n_vessels']
        
def add_vessel_type_quantities(all_results_df):
    # List of quantities to sum
    quantities_fleet = [column for column in all_results_df.columns if '_fleet' in column]
    
    new_rows = []

    # Iterate over each fuel, pathway, country, and number combination
    for (fuel, pathway, country, number), group_df in all_results_df.groupby(['Fuel', 'Pathway', 'Country', 'Number']):
        for vessel_type, vessel_names in vessels.items():
            ...
            if not vessel_type_df.empty:
                ...
                for quantity in quantities:
                    vessel_type_row[quantity] = vessel_type_row[f'{quantity}_fleet'] / vessel_type_row['n_vessels']
...
def add_fleet_level_quantities(all_results_df):
    ...
    for (fuel, pathway, country, number), group_df in all_results_df.groupby(['Fuel', 'Pathway', 'Country', 'Number']):
        ...
        if not fleet_df.empty:
            ...
            # Evaluate the average based on the fleet sum for each vessel-level quantity
            for quantity in quantities:
                fleet_row[quantity] = fleet_row[f'{quantity}_fleet'] / fleet_row['n_vessels']
...
def generate_csv_files(all_results_df, top_dir):
    quantities_of_interest = list(all_results_df.drop(columns=['Vessel', 'Fuel', 'Pathway', 'Country', 'Number', 'n_vessels']).columns)
    ...
    for fuel in unique_fuels:
        unique_pathways = all_results_df['Pathway'][all_results_df['Fuel'] == fuel].unique()
        for pathway in unique_pathways:
            filter = (all_results_df['Fuel'] == fuel) & (all_results_df['Pathway'] == pathway)
            ...
            for quantity in quantities_of_interest:
                quantity_selected_results_df = all_selected_results_df[['Vessel', 'Country', quantity]]

                # Pivot the DataFrame
                pivot_df = quantity_selected_results_df.pivot(index='Country', columns='Vessel', values=quantity)
                ...
                countries_with_multiple_entries = quantity_selected_results_df['Country'][quantity_selected_results_df['Country'].str.contains('_2')]
                base_countries_with_multiple_entries = countries_with_multiple_entries.apply(lambda x: x.split('_')[0]).unique()
                ...
                for base_country in base_countries_with_multiple_entries:
                    if base_country in pivot_df.index:
                        pivot_df.rename(index={base_country: f'{base_country}_1'}, inplace=True)

                # Calculate the average for each base country with multiple entries and add as a new row
                avg_rows = []
                for base_country in base_countries_with_multiple_entries:
                    matching_rows = pivot_df.loc[pivot_df.index.str.startswith(base_country + '_')]
