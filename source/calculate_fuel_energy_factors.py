...
def add_info_row(info_df, pattern, fuel):
    ...
    combined = lsfo_info.add(main_info, fill_value=0).select_dtypes(
        include=[float, int]
    )
...
def main():
    ...
    for fuel in fuels:
        ...
        pattern = os.path.join(
            top_dir,
            "processed_results",
            f"{fuel}-*-ConsumedEnergy_fueltype-per_mile.csv",
        )
        ...
        pattern = os.path.join(
            top_dir,
            "processed_results",
            f"{fuel}-*-ConsumedEnergy_fueltype-per_tonne_mile_lsfo.csv",
        )
        total_energy_per_tonne_mile_max_cap = add_info_row(
            total_energy_per_tonne_mile_max_cap, pattern, fuel
        )

        pattern = os.path.join(
            top_dir,
            "processed_results",
            f"{fuel}-*-ConsumedEnergy_fueltype-per_cbm_mile_lsfo.csv",
        )
        total_energy_per_cbm_mile_max_cap = add_info_row(
            total_energy_per_cbm_mile_max_cap, pattern, fuel
        )
        ###########################################################################################################################################################
        ...
        pattern = os.path.join(
            top_dir,
            "processed_results",
            f"{fuel}-*-ConsumedEnergy_fueltype-per_tonne_mile_lsfo_final.csv",
        )
        total_energy_per_tonne_mile_mod_cap = add_info_row(
            total_energy_per_tonne_mile_mod_cap, pattern, fuel
        )

        pattern = os.path.join(
            top_dir,
            "processed_results",
            f"{fuel}-*-ConsumedEnergy_fueltype-per_cbm_mile_lsfo_final.csv",
        )
        total_energy_per_cbm_mile_mod_cap = add_info_row(
            total_energy_per_cbm_mile_mod_cap, pattern, fuel
        )
        ###########################################################################################################################################################

    # Save all results to csv
    total_energy_per_mile.to_csv("tables/fuel_GJ_per_nautical_mile.csv")
    total_energy_per_tonne_mile_max_cap.to_csv(
        "tables/fuel_GJ_per_tonne_nautical_mile_max_cap.csv"
    )
    total_energy_per_cbm_mile_max_cap.to_csv(
        "tables/fuel_GJ_per_cbm_nautical_mile_max_cap.csv"
    )
    total_energy_per_tonne_mile_mod_cap.to_csv(
        "tables/fuel_GJ_per_tonne_nautical_mile_lsfo_cap.csv"
    )
    total_energy_per_cbm_mile_mod_cap.to_csv(
        "tables/fuel_GJ_per_cbm_nautical_mile_lsfo_cap.csv"
    )
