...
def add_transportation_storage_costs(costs_emissions_df, fuel, port):
    ...
    merged_df.rename(columns={"LCOF [$/tonne]": "Fuel Production LCOF [$/tonne]", "Emissions [kg CO2e / kg fuel]": "Fuel Production Emissions [kg CO2e / kg fuel]"}, inplace=True)
        ...
        if result:
            ...
            if fuel in fuels:
                ...
                for port in ports:
                    ...
                    costs_emissions_df.to_csv(os.path.join(output_dir, f"{fuel}_{port}_costs_emissions.csv"), index=False)
