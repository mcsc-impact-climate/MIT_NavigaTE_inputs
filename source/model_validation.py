...
def read_results(filename, fuel='lsfo'):
    columns = ['Vessel', 'Fuel', 'WTT Emissions (tonnes CO2 / year)', 'TTW Emissions (tonnes CO2 / year)', 'WTW Emissions (tonnes CO2 / year)', 'CAPEX (USD / year)', 'Fuel Cost (USD / year)', 'Other OPEX (USD / year)', 'Total Cost (USD / year)', 'Energy Consumed (GJ / year) [main]', 'Energy Consumed (GJ / year) [pilot]', 'Energy Spend (GJ / year)', 'Miles / year', 'Cargo tonne-miles / year']
...
def compare_fuel_consumption(all_results_df, fuel='lsfo'):
    ...
    columns = ['Vessel', 'Global annual fuel cons (NavigaTE)', 'Global annual fuel cons (IMO)', 'Global perc diff', 'Average vessel annual fuel cons (NavigaTE)', 'Average vessel annual fuel cons (IMO)', 'Vessel perc diff']
    fuel_consumption_df = pd.DataFrame(columns=columns)
    for vessel_type in vessels:
        fuel_consumption_info = {
            'Vessel': vessel_type,all_results_df = pd.DataFrame(columns=columns)
        ...
        for vessel in vessels[vessel_type]:
            fuel_consumption_info['Global annual fuel cons (NavigaTE)'] += energy_to_fuel_consumption_lsfo(float(all_results_df['Energy Spend (GJ / year)'][all_results_df['Vessel']==f'{vessel}_{fuel}'])) * vessel_size_number[vessel]
            vessel_type_number_navigate += vessel_size_number[vessel]

        fuel_consumption_info['Global perc diff'] = 100 * (fuel_consumption_info['Global annual fuel cons (NavigaTE)'] - fuel_consumption_info['Global annual fuel cons (IMO)']) / fuel_consumption_info['Global annual fuel cons (IMO)']
    
        fuel_consumption_info['Average vessel annual fuel cons (NavigaTE)'] = fuel_consumption_info['Global annual fuel cons (NavigaTE)'] / vessel_type_number_navigate
    
        fuel_consumption_info['Average vessel annual fuel cons (IMO)'] = fuel_consumption_info['Global annual fuel cons (IMO)'] / vessel_numbers_imo[vessel_type]
    
        fuel_consumption_info['Vessel perc diff'] = 100*(fuel_consumption_info['Average vessel annual fuel cons (NavigaTE)'] - fuel_consumption_info['Average vessel annual fuel cons (IMO)']) / fuel_consumption_info['Average vessel annual fuel cons (IMO)']
        
        fuel_consumption_row_df = pd.DataFrame([fuel_consumption_info])
        fuel_consumption_df = pd.concat([fuel_consumption_df, fuel_consumption_row_df], ignore_index=True)
...
def compare_fuel_consumption_rate(all_results_df, fuel='lsfo'):
    ...
    columns = ['Vessel', 'Fuel cons rate (NavigaTE)', 'Fuel cons rate (IMO)', 'Perc diff']
    fuel_consumption_rate_df = pd.DataFrame(columns=columns)
    for vessel_type in vessels:
        fuel_consumption_rate_info = {
            'Vessel': vessel_type,
            'Fuel cons rate (NavigaTE)': 0,
            'Fuel cons rate (IMO)': (annual_fuel_consumption_imo[vessel_type] / annual_cargo_miles_imo_opt2[vessel_type])      # tonnes of fuel / kilotonne-mile
        }
        vessel_type_number_navigate = 0
        for vessel in vessels[vessel_type]:
            fuel_consumption_rate_info['Fuel cons rate (NavigaTE)'] += energy_to_fuel_consumption_lsfo(float(all_results_df['Energy Spend (GJ / year)'][all_results_df['Vessel']==f'{vessel}_{fuel}'].iloc[0])) / float(all_results_df['Cargo tonne-miles / year'][all_results_df['Vessel']==f'{vessel}_{fuel}'].iloc[0]) * vessel_size_number[vessel] * TONNES_PER_KT * TONNES_PER_KT
            vessel_type_number_navigate += vessel_size_number[vessel]

        fuel_consumption_rate_info['Fuel cons rate (NavigaTE)'] = fuel_consumption_rate_info['Fuel cons rate (NavigaTE)'] / vessel_type_number_navigate
        fuel_consumption_rate_info['Perc diff'] = 100 * (fuel_consumption_rate_info['Fuel cons rate (NavigaTE)'] - fuel_consumption_rate_info['Fuel cons rate (IMO)']) / fuel_consumption_rate_info['Fuel cons rate (IMO)']
        
        fuel_consumption_rate_row_df = pd.DataFrame([fuel_consumption_rate_info])
        fuel_consumption_rate_df = pd.concat([fuel_consumption_rate_df, fuel_consumption_rate_row_df], ignore_index=True)
...
def compare_ttw_emissions(all_results_df, fuel='lsfo'):
    ...
    columns = ['Vessel', 'TTW Emission Rate (NavigaTE)', 'TTW Emission Rate (IMO Opt 1)', 'Perc diff (IMO Opt 1)', 'TTW Emission Rate (IMO Opt 2)', 'Perc diff (IMO Opt 2)']
    ...
    for vessel_type in vessels:
        ...
        vessel_type_number_navigate = 0
        for vessel in vessels[vessel_type]:
            ttw_emission_rate_info['TTW Emission Rate (NavigaTE)'] += ( float(all_results_df['TTW Emissions (tonnes CO2 / year)'][all_results_df['Vessel']==f'{vessel}_{fuel}'].iloc[0]) ) / ( float(all_results_df['Cargo tonne-miles / year'][all_results_df['Vessel']==f'{vessel}_{fuel}'].iloc[0]) ) * KG_PER_TONNE * G_PER_KG * vessel_size_number[vessel]        # Need to weight the sum by the number of vessels of each size class
            vessel_type_number_navigate += vessel_size_number[vessel]
        
        # Divide by the total number of vessels of the given type to get the average TTW emission rate
        ttw_emission_rate_info['TTW Emission Rate (NavigaTE)'] = ttw_emission_rate_info['TTW Emission Rate (NavigaTE)'] / vessel_type_number_navigate

        ttw_emission_rate_info['Perc diff (IMO Opt 1)'] = 100 * (ttw_emission_rate_info['TTW Emission Rate (IMO Opt 1)'] - ttw_emission_rate_info['TTW Emission Rate (NavigaTE)']) / ttw_emission_rate_info['TTW Emission Rate (IMO Opt 1)']
        
        ttw_emission_rate_info['Perc diff (IMO Opt 2)'] = 100 * (ttw_emission_rate_info['TTW Emission Rate (IMO Opt 2)'] - ttw_emission_rate_info['TTW Emission Rate (NavigaTE)']) / ttw_emission_rate_info['TTW Emission Rate (IMO Opt 2)']
        
        ttw_emission_rate_row_df = pd.DataFrame([ttw_emission_rate_info])

        ttw_emission_rate_df = pd.concat([ttw_emission_rate_df, ttw_emission_rate_row_df], ignore_index=True)
...
def compare_annual_miles(all_results_df, fuel='lsfo'):
    ...
    columns = ['Vessel', 'Annual miles traveled (NavigaTE)', 'Annual miles traveled (IMO)', 'Perc diff']
    miles_traveled_df = pd.DataFrame(columns=columns)
    for vessel_type in vessels:
        for vessel in vessels[vessel_type]:
            annual_miles_navigate = float(all_results_df['Miles / year'][all_results_df['Vessel']==f'{vessel}_{fuel}'])
            annual_miles_imo = annual_miles_imo_dict[vessel]
            perc_diff = 100*(annual_miles_navigate-annual_miles_imo) / annual_miles_imo
            miles_traveled_info = {
                'Vessel': vessel,
            ...
            miles_traveled_df = pd.concat([miles_traveled_df, miles_traveled_row_df], ignore_index=True)
...
def compare_annual_cargo_miles(all_results_df, fuel='lsfo'):
    ...
    columns = ['Vessel', 'Annual cargo miles (NavigaTE)', 'Annual cargo miles (IMO opt 1)', 'Perc diff (IMO opt 1)', 'Annual cargo miles (IMO opt 2)', 'Perc diff (IMO opt 2)', 'Annual cargo miles (UNCTAD)', 'Perc diff (UNCTAD)']
    ...
    for vessel_type in vessels:
        ...
        for vessel in vessels[vessel_type]:
            cargo_miles_info['Annual cargo miles (NavigaTE)'] += float(all_results_df['Cargo tonne-miles / year'][all_results_df['Vessel']==f'{vessel}_{fuel}'].iloc[0])/1e6
            
        cargo_miles_info['Perc diff (IMO opt 1)'] = 100 * (cargo_miles_info['Annual cargo miles (NavigaTE)'] - cargo_miles_info['Annual cargo miles (IMO opt 1)']) / cargo_miles_info['Annual cargo miles (IMO opt 1)']
        cargo_miles_info['Perc diff (IMO opt 2)'] = 100 * (cargo_miles_info['Annual cargo miles (NavigaTE)'] - cargo_miles_info['Annual cargo miles (IMO opt 2)']) / cargo_miles_info['Annual cargo miles (IMO opt 2)']
        cargo_miles_info['Perc diff (UNCTAD)'] = 100 * (cargo_miles_info['Annual cargo miles (NavigaTE)'] - cargo_miles_info['Annual cargo miles (UNCTAD)']) / cargo_miles_info['Annual cargo miles (UNCTAD)']
        
        cargo_miles_row_df = pd.DataFrame([cargo_miles_info])
        cargo_miles_df = pd.concat([cargo_miles_df, cargo_miles_row_df], ignore_index=True)
...
def main():
    ...
    all_results_df = read_results(f'{top_dir}/all_outputs_full_fleet/report_lsfo.xlsx', 'lsfo')
