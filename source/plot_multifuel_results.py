...
def read_results_fleet(filename):
    ...
    for vessel_type in vessels:
        ...
        for vessel in vessels[vessel_type]:
            ...
            results_dict[vessel_type][vessel]["fleet info"] = filtered_df.drop(
                index=[0, 1, 2]
            ...
            vessel_columns = [
                col
                for col in results_df_vessel.columns
                if isinstance(results_df_vessel.iloc[1][col], str)
                and vessel in results_df_vessel.iloc[1][col]
            ]
            ...
            unique_main_fuels = [
                value.split("_ice_")[-1]
                for value in unique_main_fuels_long
                if isinstance(value, str) and "ice" in value
            ]

            # Loop to create one dataframe for each unique vessel type
            for main_fuel_long, main_fuel in zip(
                unique_main_fuels_long, unique_main_fuels
            ):
                ...
                main_fuel_df.insert(
                    0,
                    "Time (days)",
                    results_df_vessel["Time (days)"].iloc[3:].reset_index(drop=True),
                )
                main_fuel_df.insert(
                    0, "Date", results_df_vessel["Date"].iloc[3:].reset_index(drop=True)
                )

                # Add the dataframe to the dictionary
                results_dict[vessel_type][vessel]["main fuel info"][main_fuel] = (
                    main_fuel_df
                )
            ...
            consumed_fuel_columns = [
                col
                for col in results_df_vessel.columns
                if isinstance(results_df_vessel.iloc[1][col], str)
                and vessel not in results_df_vessel.iloc[1][col]
            ]
            ...
            unique_consumed_fuels = (
                filtered_df.iloc[1].dropna().unique()
            )  # Drop NaN values before extracting unique names
            ...
            for consumed_fuel in unique_consumed_fuels:
                ...
                consumed_fuel_columns = [
                    col
                    for col in filtered_df.columns
                    if isinstance(filtered_df.iloc[1][col], str)
                    and filtered_df.iloc[1][col] == consumed_fuel
                ]
                ...
                if consumed_fuel_columns:
                    ...
                    # Drop rows with index 0, 1, and 2
                    consumed_fuel_df = consumed_fuel_df.drop(
                        index=[0, 1, 2]
                    ).reset_index(drop=True)

                    # Add 'Date' and 'Time (days)' columns at the far left
                    consumed_fuel_df.insert(
                        0,
                        "Time (days)",
                        results_df_vessel["Time (days)"]
                        .iloc[3:]
                        .reset_index(drop=True),
                    )
                    consumed_fuel_df.insert(
                        0,
                        "Date",
                        results_df_vessel["Date"].iloc[3:].reset_index(drop=True),
                    )
...
def read_results_global(filename):
    ...
    filtered_columns = results_df.columns[
        (results_df.loc[1].isna())
        & (results_df.loc[2].isna())
        & ~(results_df.loc[0].isna())
    ]
    ...
    results_dict["fleet info"] = pd.concat(
        [
            results_df[["Date", "Time (days)"]].iloc[3:].reset_index(drop=True),
            filtered_df,
        ],
        axis=1,
    )
    ...
    for fuel in fuel_names:
        ...
        fuel_columns = results_df.columns[
            (results_df.iloc[1] == fuel) & (results_df.iloc[2].isna())
        ]
        ...
        fuel_df.rename(
            columns={col: results_df.at[0, col] for col in fuel_columns}, inplace=True
        )
...
def read_results_regulation(filename):
    ...
    regulation_names = [
        name for name in regulation_names if name not in ["Date", "Time (days)"]
    ]
    ...
    for regulation in regulation_names:
        ...
        regulation_columns = [
            col for col, name in col_regulation_mapping.items() if name == regulation
        ]
        ...
        col_names = ["Date", "Time (days)"] + [
            f"{results_df.iloc[1, col]}" for col in regulation_columns
        ]
        ...
        fleet_columns = [
            col_names[i]
            for i, col in enumerate(regulation_columns, start=2)
            if pd.isna(results_df.iloc[2, col])
        ]
        ...
        for vessel in unique_vessels:
            ...
            vessel_col_indices = [
                col
                for col in range(regulation_df.shape[1])
                if regulation_df.iloc[2, col] == vessel
            ]
        ...
        results_dict[regulation] = {"fleet info": fleet_df, "vessel info": vessel_info}

    return results_dict


def plot_fleet_info_column(
    data_dict, column_name, info_type="global", ylabel=None, save_label=None
):
...
def plot_regulation_vessel_info(
    data_dict,
    column_name,
    regulation_name="net_zero_regulation",
    vessel_name="container_3500_teu_ice_liquid_hydrogen",
    ylabel=None,
    save_label=None,
):
    ...
    # Save the plot
    print(
        f"Saving to plots/multifuel/regulation_{regulation_name}_{vessel_name}_{column_name}{save_label}.png"
    )
    plt.savefig(
        f"plots/multifuel/regulation_{regulation_name}_{vessel_name}_{column_name}{save_label}.png",
        dpi=300,
    )

    print(
        f"Saving to plots/multifuel/regulation_{regulation_name}_{vessel_name}_{column_name}{save_label}.pdf"
    )
    plt.savefig(
        f"plots/multifuel/regulation_{regulation_name}_{vessel_name}_{column_name}{save_label}.pdf"
    )
    plt.close()
...
def plot_global_stacked_fuel_info(
    global_results_dict, column_name, ylabel=None, save_label=None
):
...
def plot_main_fuel_info_fleet(
    fleet_results_dict,
    column,
    vessel_classes=["bulk_carrier", "container", "tanker", "gas_carrier"],
    level="size",
    ylabel=None,
    save_label=None,
):
    ...
    for vessel_class in vessel_classes:
        ...
        for vessel_size in fleet_results_class_dict:
            main_fuel_size_dict = fleet_results_class_dict[vessel_size][
                "main fuel info"
            ]
            ...
            if stacked_data is not None:
                ...
                sorted_cols, color_list, label_list = sort_color_label_columns(
                ...
                if level == "size":
                    ...
                    save_path_png = (
                        f"plots/multifuel/size_{vessel_size}_{column}{save_label}.png"
                    )
                    save_path_pdf = (
                        f"plots/multifuel/size_{vessel_size}_{column}{save_label}.pdf"
                    )
        ...
        if level == "class" and class_stacked_data is not None:
            ...
            save_path_png = (
                f"plots/multifuel/class_{vessel_class}_{column}{save_label}.png"
            )
            save_path_pdf = (
                f"plots/multifuel/class_{vessel_class}_{column}{save_label}.pdf"
            )
        ...
        if level == "fleet" and class_stacked_data is not None:
            if fleet_stacked_data is None:
                ...
                fleet_stacked_data = fleet_stacked_data.add(
                    class_stacked_data, fill_value=0
                )

    if level == "fleet" and fleet_stacked_data is not None:
        fleet_stacked_data = fleet_stacked_data.fillna(0)
        sorted_cols, color_list, label_list = sort_color_label_columns(
            fleet_stacked_data.columns
        )
...
def read_results_vessel(filename, sheet_name="Vessels"):
    ...
    df_data = df_raw.iloc[5:].reset_index(
        drop=True
    )  # Skip header rows, reset row index for clean DataFrame

    # === 3. Extract header rows to identify column information ===
    row0 = df_raw.iloc[0]  # Full vessel+fuel names
    row1 = df_raw.iloc[
        1
    ]  # Primary measurement type (e.g., ConsumedEnergy, TotalEquivalentWTT)
    ...
    for base_vessel, fuels_dict in vessel_bases.items():
        ...
        for fuel, col_indices in fuels_dict.items():
            ...
            for cidx in col_indices:
                new_col_name = col_name_map.get(
                    cidx, f"Unknown_{cidx}"
                )  # Use the rename map
...
def plot_vessel_fuel_metric(
    vessel_results_dict,
    column_name,
    xlabel=None,
    save_label=None,
    relative_to_lsfo=False,
):
    """
    ...
    for vessel in vessel_names:
        ...
        for fuel, df in fuel_dict.items():
            ...
            if relative_to_lsfo:
                ...
                ax.scatter(
                    value, y_pos, color=color, label=label, s=150, edgecolors="k"
                )
    ...
    plt.subplots_adjust(
        right=0.75
    )  # Shrinks the plot area to the left, leaves space on right
    ...
    if relative_to_lsfo:
        save_path_png = (
            f"plots/multifuel/vessel_fuel_{column_name}{save_label}_rel_to_lsfo.png"
        )
        save_path_pdf = (
            f"plots/multifuel/vessel_fuel_{column_name}{save_label}_rel_to_lsfo.pdf"
        )
    else:
...
def plot_vessel_fuel_stacked_histograms(
    vessel_results_dict,
    column_names,
    column_labels,
    xlabel=None,
...
def make_all_plots(results_file, results_label=None, regulations=None):
    ...
    plot_main_fuel_info_fleet(
        fleet_results_dict,
        "ExistingVessels",
        level="fleet",
        ylabel="Existing Vessels",
        save_label=results_label,
    )
    plot_main_fuel_info_fleet(
        fleet_results_dict, "Newbuilds", level="fleet", save_label=results_label
    )
    plot_main_fuel_info_fleet(
        fleet_results_dict,
        "Scrap",
        level="fleet",
        ylabel="Scrapped Vessels",
        save_label=results_label,
    )

    # Plot vessel metrics by fuel
    vessel_results_dict = read_results_vessel(results_file)
    plot_vessel_fuel_metric(
        vessel_results_dict,
        "InvestmentMetricExpected",
        xlabel="Vessel Investment Metric",
        save_label=results_label,
        relative_to_lsfo=True,
    )
    plot_vessel_fuel_metric(
        vessel_results_dict,
        "CargoMiles",
        xlabel="Annual Vessel Cargo Miles",
        save_label=results_label,
        relative_to_lsfo=True,
    )
    plot_vessel_fuel_metric(
